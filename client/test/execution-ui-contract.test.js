...
test('manual execution UI is accessible and keeps live authority explicit', () => {
  ...
  assert.match(source, /自动模拟盘.*4002/)
  assert.match(source, /IBKR 模拟盘自动执行/)
  assert.match(source, /kind: 'validate_plan'/)
  assert.match(source, /kind: 'start', confirmation/)
  assert.match(source, /今日冻结计划/)
  assert.match(source, /Agent 只能影响安全包/)
