...
class PaperAutopilot:
    ...
    def _require_write_gates(self) -> None:
        if not _boolean_environment(
            self.environ,
            "BROKER_WRITE_ENABLED",
            default=False,
        ):
            raise RuntimeError("BROKER_WRITE_ENABLED must be true before paper start")
        if _boolean_environment(
            self.environ,
            "TRADING_KILL_SWITCH",
            default=True,
        ):
            raise RuntimeError("TRADING_KILL_SWITCH must be false before paper start")
