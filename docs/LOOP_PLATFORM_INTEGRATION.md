...
- `realized_policy_return`：有可核验成交时才填写实际策略净收益，未成交或未关联成交为 0；
...
`reject/block` 的反事实手续费和滑点采用正常往返成本的 2 倍。旧
...
Reporter 只在字段完整时写入 `policy_evaluation`。Loop 的 Doubly Robust 奖励是显式
