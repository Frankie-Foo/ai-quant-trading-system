...
答疑和 Agent 不是本地模型：调用时会把白名单化、最小化的选股/复盘证据发送至
OpenRouter 及用户选择的模型提供商。普通答疑不发送任务账本和成熟度；只有证据审计
Agent 会发送去除 token、密钥和原始载荷后的任务与成熟度字段。每个返回结果都携带
交易日、选股快照 ID、证据时间和复盘快照 ID，客户端会标记证据已切换的旧结论。
