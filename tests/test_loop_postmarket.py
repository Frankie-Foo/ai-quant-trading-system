...
def test_completed_local_review_retries_blocked_loop(
    ...
    if with_provider == "native":
        ...
        plan = tmp_path / "runs" / "autonomous" / target.isoformat() / "modern_h15_paper_plan.json"
...
def test_missing_native_plan_submits_audit_only_review_without_provider(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch,
) -> None:
    target = date(2026, 7, 20)
    ledger = JobLedger(tmp_path / "jobs.sqlite3")
    ...
    def child(command: list[str], **kwargs: object) -> ChildProcessResult:
        ...
        assert "scripts.produce_loop_daily" not in command
        assert "scripts.sync_loop_daily_review" in command
        assert "--execution-index" not in command
        return ChildProcessResult(
            return_code=0,
